...
$('#reWriteBtn').click(function(){
   var msg=$('#rmsg').val();
   if($("#play").get(0).textContent!="play!")
   {
      alert("광고시청중에는 댓글을 달 수 없습니다");
   }
   else if($("#play").get(0).textContent=="play!")
   {
      if(msg=="")
      {
         alert("댓글내용을 입력하세요");
...
{
   var rmsg=$('#rmsg'+no).val();
   if($("#play").get(0).textContent="pause!")
   {
      if(rmsg=="")
      {
         alert("댓글 내용을 입력하세요");
...
<body id="body">
   <div id="body">
      <div id="body_content">
         ...
         <button onclick="playPause()" id="play" style="margin-left:5%;width:330px;height:50px;"name="pbutton">play!</button> 
         <button id="getpoint" disabled="disabled" style="width:330px;height:50px;" name="pbutton"
            onclick="getpoint('<%=request.getParameter("adno")%>', '${adinfo.ppc}', '${adinfo.adsubject}')">포인트 적립</button>
         <br><br>
      </div>
      <div id="body_recommend">
